%% set range of posiible intensities to test for the threshold

% *** remove all notes and variables which belong to the tms_configure script ***
...
% intensity here
tms.intensity.range = tms.intensity.min : tms.resolution: tms.intensity.max;% MSO	*** this should be set in the tms_configure script ***


% index here
tms.index.range = 1:length(tms.intensity.range);                            % position in array *** this should be set in the tms_configure script ***

% max trial number for same intensity
tms.max_same = quest.decision_trials;						% *** this only applies if the quest algorithm is being used ***
...
% save variables
tms.maxtrial = 10;								% *** what does this do? ***

										% *** why do all of these use QUEST parameters - should not happen true when QUEST is not used
